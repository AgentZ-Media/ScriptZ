...
Vom Repo-Root (Monorepo):

```bash
pnpm install              # installiert Desktop und Landing
pnpm dev:desktop          # Hot-Reload Frontend + Rust
pnpm build:desktop        # Native .app bauen
pnpm typecheck            # TypeScript über alle Apps
```

Innerhalb dieses Verzeichnisses funktionieren auch die App-eigenen
Skripte:
...
Code-Konventionen siehe [CLAUDE.md](CLAUDE.md).
